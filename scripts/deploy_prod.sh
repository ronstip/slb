...
gcloud run deploy sl-api \
    ...
    --set-env-vars "^|^ENVIRONMENT=production|GCP_PROJECT_ID=$PROJECT_ID|GCP_REGION=$REGION|GOOGLE_GENAI_USE_VERTEXAI=TRUE|GOOGLE_CLOUD_PROJECT=$PROJECT_ID|GOOGLE_CLOUD_LOCATION=global|ENABLE_SEARCH_GROUNDING=true|SIGNUP_GATE=entitlements|SUPER_ADMIN_EMAILS=saharmalka@gmail.com,ronneeman19@gmail.com|VETRIC_API_KEY_TWITTER=$VETRIC_API_KEY_TWITTER|VETRIC_API_KEY_INSTAGRAM=$VETRIC_API_KEY_INSTAGRAM|VETRIC_API_KEY_TIKTOK=$VETRIC_API_KEY_TIKTOK|HIKERAPI_API_KEY=$HIKERAPI_API_KEY|SENTRY_DSN=${SENTRY_DSN:-}|SENTRY_ENVIRONMENT=production|SENTRY_RELEASE=$RELEASE_SHA" \
    --min-instances 1 \
    --max-instances 10 \
    --memory 1Gi \
    --cpu 1 \
