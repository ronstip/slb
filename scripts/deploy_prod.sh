...
gcloud run deploy sl-worker \
    ...
    --set-env-vars "^|^ENVIRONMENT=production|GCP_PROJECT_ID=$PROJECT_ID|GCP_REGION=$REGION|GOOGLE_GENAI_USE_VERTEXAI=TRUE|GOOGLE_CLOUD_PROJECT=$PROJECT_ID|GOOGLE_CLOUD_LOCATION=global|BRIGHTDATA_API_TOKEN=$BRIGHTDATA_API_TOKEN|X_API_BEARER_TOKEN=$X_API_BEARER_TOKEN|APIFY_API_TOKEN=$APIFY_API_TOKEN|APIFY_ACTOR_INSTAGRAM=apidojo/instagram-hashtag-scraper|APIFY_ACTOR_INSTAGRAM_POST=apify/instagram-scraper|APIFY_ACTOR_FACEBOOK=scrapeforge/facebook-search-posts|APIFY_ACTOR_FACEBOOK_PAGE=apify/facebook-posts-scraper|APIFY_ACTOR_TIKTOK=clockworks/tiktok-scraper|DEFAULT_VENDOR_INSTAGRAM=apify|DEFAULT_VENDOR_FACEBOOK=apify|DEFAULT_VENDOR_TIKTOK=apify|ENRICHMENT_MODEL=gemini-3-flash-preview|ENRICHMENT_VIDEO_FPS=0.5|ENRICHMENT_VIDEO_END_OFFSET=180s|ENRICHMENT_MAX_OUTPUT_TOKENS=4096|VETRIC_API_KEY_TWITTER=$VETRIC_API_KEY_TWITTER|VETRIC_API_KEY_INSTAGRAM=$VETRIC_API_KEY_INSTAGRAM|VETRIC_API_KEY_TIKTOK=$VETRIC_API_KEY_TIKTOK|SENTRY_DSN=${SENTRY_DSN:-}|SENTRY_ENVIRONMENT=production|SENTRY_RELEASE=$RELEASE_SHA" \
    --min-instances 0 \
    --max-instances 5 \
    --memory 2Gi \
    --cpu 2 \
