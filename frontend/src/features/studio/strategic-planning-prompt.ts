...
1. **Header / metadata block** — period covered, scope, total posts analyzed, platform mix (with %), language mix, total reach, monitoring-agent ID, primary entities tracked, and a 2–3 line *contextual frame*: "what happened in the world during this period that matters." This sets the stakes.

2. **Executive summary** — opens with the single most important insight given the user's framing, then **4–6 callout findings**. Each callout is one short, hard-claim paragraph that names actors, numbers, and direction. Use bold for the load-bearing words. No table of contents, no preamble. Close the section with **5 operational recommendations for the next period** in a numbered list — each with quantitative justification (the specific number/finding that motivates it), a target date or window, and a concrete execution template.

3. **KPI dashboard / Share-of-Voice table** — per actor or per cohort: mention count, total reach/views, share-of-voice %, dominant sentiment with pro/anti split. One row per entity, ranked by reach. Add 1–2 paragraphs of *strategic insight* beneath the table that interpret the asymmetries (who leads in volume vs. reach, who has the worst pro/anti ratio, who is silent).

4. **Day-by-day or period-by-period chronology** — a row per day with posts, total reach, sentiment split, and a one-line dominant emotion or daily inflection. Below the table, name the **inflection points** — the 2–3 days that changed the shape of the period and what drove them.

5. **Engagement decomposition** — at minimum two cuts:
...
6. **Per-actor competitive positioning** — one short sub-section per primary actor in scope (typically the subject + 2–4 main rivals or peers). Each covers: dominant narrative this period, top 2–4 posts (with date, time, format, views, likes, message, *why it worked*), 2–4 critical weaknesses, and what they missed. Bold the asymmetric findings.

7. **Top N posts table for the subject** — 5–7 best posts: date, time, platform, format, views, likes, the message, **why it worked**, and a **replication template** the campaign can re-use next period. The replication template is what turns the table from observation into prescription.

8. **Emotion / message analysis of the subject's own content** — when the data has emotion enrichment, count which emotions correlate with strong performance and which under-perform. State the implication for tone and framing in the next period.

9. **What was missed** — a candid list of opportunities the subject did not capitalize on. Each item names the specific opportunity, the cost (in lost reach or narrative ground), and why it matters.

10. **Narratives, clusters, and hashtags** — a table of the live narrative clusters: cluster name, post count, lead voices (specific handles), status (emerging / sustained / fading / dangerous), recommended response. If the data has topic clusters via list_topics, use them by name. Track branded hashtag adoption explicitly and call out gaps (the canonical example: the campaign's own hashtag has 0 appearances in 1,000+ posts — that is a finding, not a footnote).

11. **Platform comparison** — when the data spans multiple platforms, an explicit comparison: post share, reach share, sentiment difference, audience-age implication. Make the asymmetry strategic, not descriptive.

12. **External trends that shaped the period** — events outside the social data that explain spikes, shifts, or absences. Use web grounding to verify and cite at least 3–5 external sources here with links. Tie each external event to a specific data signal in the report.

13. **Audience insights** — who is actually doing the talking: dominant cohorts, named influencer accounts (handles), audience overlaps with adjacent actors. When two cohorts overlap unexpectedly, that is the finding. Identify potential persuasion targets vs. lost causes.

14. **Risks & opportunities matrix** — two short tables. **Risks**: name, area, urgency tag (critical / high / medium / low), recommended action. **Opportunities**: name, size estimate, recommended next move, target date. Operational, not philosophical.

15. **Operational recommendations — detailed** — the long form of the 5 recommendations from the executive summary. Each recommendation gets its own sub-section: quantitative justification (the specific finding this rests on), execution plan (a calendar table for content cadence where applicable, with day / time / channel / format / template), specific accounts / formats / times to target, and a measurement KPI for whether it worked. Generic recommendations ("increase engagement", "use more video") are a tell that the analysis was thin.

16. **Appendix A — external context** — polls, press articles, web research, market data, third-party reports that ground the analysis. **Mandatory and substantial**: at least 5 external citations with one-line summaries and links. Group by type (polls / press / market / etc.) when there are enough.

17. **Appendix B — methodology and sources** — data scope, time range, total posts (and after dedup), platform/language mix, classification taxonomy used (sentiment categories, stance values, topic-cluster count), monitoring-agent ID and source-collection IDs, list of external sources consulted. Be transparent about what the data does and does not cover.
...
Direct, operational, decision-ready. The reader is a senior decision-maker, not a curious browser. Lead claims; prove with numbers and posts; recommend actions. No hedging adverbs. No "it could be argued." No throat-clearing. No final "in conclusion" paragraph — close with Appendix B.
