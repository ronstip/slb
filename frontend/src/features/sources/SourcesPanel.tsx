...
export function SourcesPanel() {
  ...
  // Filter out collections that are already in sources
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-border-default/60 px-3 py-2">
        ...
        <button
          onClick={toggle}
          className="rounded-lg p-1 text-text-tertiary transition-colors hover:bg-bg-surface hover:text-text-secondary"
        >
          {collapsed ? (
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="relative p-3 pb-0" ref={menuRef}>
            <button
              ...
              className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-border-default/60 bg-bg-surface px-3 py-2 text-xs font-medium text-text-primary shadow-sm transition-colors hover:bg-bg-surface-secondary"
            >
            ...
            {menuOpen && (
              <div className="absolute left-3 right-3 top-full z-10 mt-1 rounded-xl border border-border-default/60 bg-bg-surface py-1 shadow-lg">
                <button
                  onClick={() => { openModal(); setMenuOpen(false); }}
                  className="w-full rounded-lg px-3 py-2 text-left text-sm text-text-primary transition-colors hover:bg-bg-surface-secondary"
                >
                  New Collection
                </button>
                <button
                  onClick={() => { setIdInputOpen(true); setIdError(null); setMenuOpen(false); }}
                  className="w-full rounded-lg px-3 py-2 text-left text-sm text-text-primary transition-colors hover:bg-bg-surface-secondary"
                >
                ...
                <button
                  disabled
                  className="w-full px-3 py-2 text-left text-sm text-text-tertiary"
                >
                  Upload Document <span className="text-xs opacity-60">(Soon)</span>
                </button>
              </div>
            )}
          </div>
            ...
            <div className="px-3 pt-2">
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  autoFocus
                  className="min-w-0 flex-1 rounded-lg border border-border-default/60 bg-bg-surface px-2.5 py-1.5 text-xs text-text-primary placeholder:text-text-tertiary focus:border-accent focus:outline-none"
                />
                <button
                  onClick={handleAddById}
                  disabled={idLoading || !collectionIdValue.trim()}
                  className="rounded-lg bg-accent px-2.5 py-1.5 text-xs font-medium text-white transition-colors hover:bg-accent/90 disabled:opacity-40"
                >
                  {idLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Add'}
                </button>
                <button
                  onClick={() => { setIdInputOpen(false); setCollectionIdValue(''); setIdError(null); }}
                  className="rounded-lg p-1.5 text-text-tertiary transition-colors hover:bg-bg-surface-secondary hover:text-text-secondary"
                >
...
function PreviousCollectionItem({
  ...
  return (
    <button
      onClick={onAdd}
      className="group flex w-full items-start gap-2.5 rounded-xl border border-border-default/30 bg-bg-surface/60 p-2.5 text-left transition-all hover:border-accent/30 hover:bg-bg-surface hover:shadow-sm"
    >
      <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-bg-surface-secondary text-text-tertiary group-hover:bg-accent/10 group-hover:text-accent">
      ...
      <span className="mt-0.5 shrink-0 rounded-md bg-accent/8 px-1.5 py-0.5 text-[10px] font-medium text-accent opacity-0 transition-opacity group-hover:opacity-100">
        Add
      </span>
    </button>
  );
