...
import { updateTask as patchTask, runTask } from '../../../api/endpoints/tasks.ts';
...
export function ScheduleDialog({ task, open, onOpenChange }: ScheduleDialogProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await patchTask(task.task_id, updates as Parameters<typeof patchTask>[1]);
      if (editRunNow) {
        try { await runTask(task.task_id); } catch { /* 409 = already running */ }
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Set how often this task runs automatically</DialogDescription>
        </DialogHeader>
